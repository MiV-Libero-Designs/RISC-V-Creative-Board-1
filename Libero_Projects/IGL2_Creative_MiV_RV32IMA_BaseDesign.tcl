...
set Libero_project_name_CFG1 IGL2_Creative_MiV_RV32IMA_CFG1_BaseDesign

set project_folder_name_CFG2 MiV_CFG2_BD
set project_dir_CFG2 "./$project_folder_name_CFG2"
set Libero_project_name_CFG2 IGL2_Creative_MiV_RV32IMA_CFG2_BaseDesign
...
if {"$config" == "CFG1"} then {
	if {[file exists $project_dir_CFG1] == 1} then {
		...
		source ./import/components/IMA_CFG1/import_component_and_constraints_igl2_creative_rv32ima_cfg1.tcl
		save_project
        base_design_built
	}
	...
	if {[file exists $project_dir_CFG2] == 1} then {
		...
		download_cores_all_cfgs
		source ./import/components/IMA_CFG2/import_component_and_constraints_igl2_creative_rv32ima_cfg2.tcl
		save_project
        base_design_built
	}
} elseif {"$config" != ""} then {
...
configure_tool -name {SYNTHESIZE} -params {BLOCK_MODE:false} -params {BLOCK_PLACEMENT_CONFLICTS:ERROR} -params {BLOCK_ROUTING_CONFLICTS:LOCK} -params {CLOCK_ASYNC:200} -params {CLOCK_DATA:5000} -params {CLOCK_GLOBAL:2} -params {PA4_GB_COUNT:16} -params {PA4_GB_MAX_RCLKINT_INSERTION:16} -params {PA4_GB_MIN_GB_FANOUT_TO_USE_RCLKINT:300} -params {RAM_OPTIMIZED_FOR_POWER:0} -params {RETIMING:false} -params {SYNPLIFY_OPTIONS:} -params {SYNPLIFY_TCL_FILE:}
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		export_prog_job \
			-job_file_name {IGL2_Creative_MiV_RV32IMA_CFG1_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {IGL2_Creative_MiV_RV32IMA_CFG2_BaseDesign} \
